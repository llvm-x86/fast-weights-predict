...
| A2 | Carefully prepared sections, figures, tables, references | ✅ Title, Abstract, §1–§7, Tables 1–7, 16 references, and a disclosures section are present. |
...
| Item | Requirement | Status |
|---|---|---|
| B1 | LaTeX (or PDF) source — **Markdown is not accepted** | ✅ `paper.tex` (11pt article, `amsmath`/`booktabs`/`hyperref`) compiles cleanly to `paper.pdf` (17 pages, letter, 1" margins; zero errors, zero undefined references). |
...
- **Comments (optional):** `17 pages, 7 tables. Code: https://github.com/llvm-x86/fast-weights-predict`
...
1. Upload the **LaTeX source** (`paper.tex`, not the `.md`, and not the PDF alone) so arXiv can
   build it. The PDF has already been compiled and verified locally (`paper.pdf`, 17 pages).
